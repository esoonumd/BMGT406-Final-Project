...
if (tableExists($db, $name_of_table)) { 
	...
	if (!$result) {
		...
		$body .="<legend>Signup Information</legend><table class=\"table table-bordered\">";
		$body .="<tr><th>Signup ID </th><th>Event ID</th><th>User Email</th></tr>";
		
		if($numberOfRows) {
			foreach($numberOfRows as $resultRow) {
			// Using a foreach loop to iterate through each row of result that is returned.
			// Display the signup information (SID, eventID, email)
				$body .= "<tr><td>{$resultRow['SID']}</td>";
				$body .= "<td>{$resultRow['gameID']}</td>";
